...
Itemset.prototype.fillOut = function fillOut(itemsets, grammar) {
	...
	while (queue.length > 0) {
		...
		if (!this.exists(dotted)) { //} && dotted.getSymbol()) {
			...
			// This means we have progressed to the end of the production. There will be nothing to fill out.
			// Since we've added the dotted production to the list of dotted productions, we're done. Bail out
			// to show this is a special case.
			if (!symbol) continue;
