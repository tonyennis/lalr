 ...
 * @returns {*}
 */
Reductions.prototype.reduce = function (production, context, stack, sourceLine, options) {
	if (production.reduce) {
		return production.reduce(context, stack, sourceLine, options);
		if (!context.getHandle())
			throw new Error("Assertion, missing handle: "+context);
	}
